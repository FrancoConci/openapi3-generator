...
const getSkipMessage = filePath => {
  return `Skipping existing file: ${filePath}`;
}
...
const deleteFolders = config => new Promise((resolve, reject) => {
  try {
    if (config.deleteFolders) {
      ...
      findRemoveSync(config.target_dir, {
        dir: config.deleteFolders
      });
    }
    resolve();
  } catch (e) {
...
const generateFile = options => new Promise((resolve, reject) => {
  ...
  fs.readFile(path.resolve(root, file_name), 'utf8', (err, content) => {
    ...
    try {
      ...
      if (!skipFile) {
        newFiles[generated_path] = generated_path;
        fs.writeFile(generated_path, parsed_content, 'utf8', (err) => {
          if (err) return reject(err);
          resolve();
        });
      } else {
        if (!newFiles[generated_path]) {
          const errorMessage = getSkipMessage(generated_path);
          if (!ignoreMessages[errorMessage]) {
            console.warn(yellow(errorMessage));
            ignoreMessages[errorMessage] = errorMessage;
          }
        }
        resolve();
      }
...
const getTargetFileName = (config, operation_name) => {
...
const generateOperationFile = (config, operation, operation_name) => new Promise((resolve, reject) => {
  fs.readFile(path.join(config.root, config.file_name), 'utf8', (err, data) => {
    if (err) return reject(err);
    const target_file = getTargetFileName(config, operation_name);
    ...
    const skipFile = config.skipExistingFiles && fs.existsSync(target_file);
    if (!skipFile) {
      newFiles[target_file] = target_file;
      fs.writeFile(target_file, content, 'utf8', (err) => {
        if (err) return reject(err);
        resolve();
      });
    } else {
      if (!newFiles[target_file]) {
        const errorMessage = getSkipMessage(target_file);
        if (!ignoreMessages[errorMessage]) {
          console.warn(yellow(errorMessage));
          ignoreMessages[errorMessage] = errorMessage;
        }
    ...
    try {
      if (stats.name.includes('$$path$$')) {
        ...
        await generateOperationFiles({
          ...
          skipExistingFiles: config.skipExistingFiles
        });
