...
const schemaValidator = require('./../validation/validationSchemas');
...
router.{{@key}}('{{../../subresource}}', async (req, res, next) => {
  ...
  try {
    const errorcodes = schemaValidator.validateObjectAgainstSchema(req.body,'{{../operationId}}Request'); 
    if (errorcodes.length > 0) {
      return res.status(200).send(errorcodes);
    }
